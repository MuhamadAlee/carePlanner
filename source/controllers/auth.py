...
def create_access_token(data: dict, tenant:str):
    to_encode = model_to_dict(data)
    # converting datetime to str for encoding
    ...
    to_encode.update({'tenant': tenant})
    to_encode.update({"iat": generated_at, "exp": expires_at})
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY, ALGORITHM)

    return {"tenant": tenant, "user_id": to_encode['user_id'], "role_id": to_encode['role_id'], "is_active":to_encode['is_active'], "access_token": encoded_jwt, "token_type": "bearer"}
