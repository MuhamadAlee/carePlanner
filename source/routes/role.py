...
from controllers.role import create_role, get_role, update_role, delete_role,\
    get_all_roles, get_role_by_name, first_time_role_creation
...
async def populate_super_role():
    ...
    if not db_role:
        ...
        first_time_role_creation(db, super_admin_role)
...
@role_router.get("/get_role_by_tenant/{role_id}/{tenant_slug}/", response_model=RoleResponse, dependencies=[Depends(get_current_user)])
def read_by_tenant(tenant_slug:str, role_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user),_=Depends(is_superuser_required)):
    db = get_db_by_tenant(tenant_slug)
    return get_role(next(db), role_id)
